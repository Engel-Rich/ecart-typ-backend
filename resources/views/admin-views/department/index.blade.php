...
@section('title',\App\Utils\translate('departement_list'))
        ...
        <div class="row align-items-center mb-3">
            <div class="col-sm mb-2 mb-sm-0">
                <h1 class="page-header-title d-flex align-items-center g-2px text-capitalize"><i
                        class="tio-filter-list"></i> {{\App\Utils\translate('departement_list')}}
                    <span class="badge badge-soft-dark ml-2">{{$departements->total()}}</span>
                </h1>
            </div>
        </div>
                    ...
                    <div class="card-header">
                        <div class="row justify-content-between align-items-center flex-grow-1">
                            ...
                            <div class="col-12 col-md-6">
                                <a href="{{route('admin.departement.create')}}" class="btn btn-primary float-right"><i
                                        class="tio-add-circle"></i> {{\App\Utils\translate('add_new_departement')}}
                                </a>
                            </div>
                        </div>
                    </div>
                        ...
                        <table class="table table-borderless table-thead-bordered table-nowrap table-align-middle card-table">
                            ...
                            @foreach($departements as $key=>$departement)
                                <tr>
                                    ...
                                    <td>
                                        <a class="text-primary" href="{{ route('admin.departement.edit',[$departement['id']]) }}">
                                            {{ $departement->name }}
                                        </a>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <a class="btn btn-white mr-1"
                                            href="{{route('admin.departement.edit',[$departement['id']])}}">
                                            <span class="tio-edit"></span>
                                        </a>
                                        <a class="btn btn-white mr-1 form-alert" href="javascript:"
                                           data-id="departement-{{$departement['id']}}"
                                           data-message="{{ \App\Utils\translate('Want to delete this departement') }}?"><span class="tio-delete"></span></a>
                                            <form action="{{route('admin.departement.delete',[$departement['id']])}}"
                                                    method="post" id="departement-{{$departement['id']}}">
                                                @csrf @method('delete')
                                            </form>
                                    </td>
                                </tr>
